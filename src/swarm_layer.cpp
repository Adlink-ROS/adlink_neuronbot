...
PLUGINLIB_EXPORT_CLASS(neuronbot_swarm_layer::SwarmLayer, costmap_2d::Layer)

using costmap_2d::NO_INFORMATION;
using costmap_2d::LETHAL_OBSTACLE;
using costmap_2d::FREE_SPACE;

namespace neuronbot_swarm_layer
{
    void SwarmLayer::onInitialize()
    {
        ...
        // Dynamic recofigure
        server_ = new dynamic_reconfigure::Server<adlink_neuronbot::SwarmLayerConfig>(nh);
        f_ = boost::bind(&SwarmLayer::configure, this, _1, _2);
        server_->setCallback(f_);
    }
    
    void SwarmLayer::robotsCallback(const adlink_neuronbot::MultiRobots& robots) 
    {
    ...
    {
        std::list<adlink_neuronbot::Robot>::iterator r_it;
        
        for(r_it = transformed_robots_.begin(); r_it != transformed_robots_.end(); ++r_it)
        {
            adlink_neuronbot::Robot robot = *r_it;
    ...
    void SwarmLayer::updateBounds(double origin_x, double origin_y, double origin_z, double* min_x, double* min_y, double* max_x, double* max_y)
    {
        ...
        {
            adlink_neuronbot::Robot& robot = robots_list_.robots[i];
            adlink_neuronbot::Robot robot_tmp = robot; // copy of the original one
    ...
    void SwarmLayer::configure(adlink_neuronbot::SwarmLayerConfig &config, uint32_t level)
    {
